...
public class TestLuceneDelete4 {
	/**
	 * @author liufukun
	 */
	public static void main(String[] args) {
		...
		new Thread(new SearchConcurrent("Beijing thegodofwar",latch)).start();
		new Thread(new SearchConcurrent("wahaha",latch)).start();
		try {
			latch.await();
...
class SearchConcurrent implements Runnable {
	IndexReader reader;
	IndexSearcher searcher;
	String content;
	CountDownLatch latch;
	SearchConcurrent(String content,CountDownLatch latch) {
		try {
			reader=IndexReader.open(FSDirectory.open(new File("d://text_lucene_delete"),null));
