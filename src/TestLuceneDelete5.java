...
public class TestLuceneDelete5 {
	/**
	 * @author liufukun
	 */
	public static void main(String[] args) {
		...
		new Thread(new DeleteIndexConcurrentNew("1",latch)).start();
		new Thread(new DeleteIndexConcurrentNew("3",latch)).start();
		try {
			latch.await();
...
class DeleteIndexConcurrentNew implements Runnable {
	IndexReader reader;;
	String id;
	CountDownLatch latch;
	DeleteIndexConcurrentNew(String id,CountDownLatch latch) {
	    try {
